...
var _ = BeforeEach(func() {
	...
	ProxyAddress, tx, Proxy, err = upgradeability.DeployAdminUpgradeabilityProxy(BankAccount.TransactOpts(), Backend, WalletAddress, Owner.Address(), nil)
	Expect(err).ToNot(HaveOccurred())
	Backend.Commit()
	Expect(isSuccessful(tx)).To(BeTrue())

	ProxyWallet, err := bindings.NewWallet(ProxyAddress, Backend)
	tx, err = ProxyWallet.Initialize(Owner.TransactOpts(), Owner.Address(), true, ENSRegistryAddress, TokenWhitelistName, ControllerName, LicenceName, EthToWei(100))
})
