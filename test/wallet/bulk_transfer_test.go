...
var _ = Describe("bulk_transfer", func() {
	...
	When("the wallet has enough ETH and a spendLimit of 1 ETG", func() {
		BeforeEach(func() {
					...
					When("the tokens are added to the oracle", func() {
						...
						When("rates are not updated (=0)", func() {
							It("Should revert", func() {
								...
								Expect(isGasExhausted(tx, 81000)).To(BeFalse())
								Expect(isSuccessful(tx)).To(BeFalse())
								Expect(TestRig.LastExecuted()).To(MatchRegexp(`.*convert\(_asset, _amount\);`))
							})
						})
						...
						When("rates are updated but the spendLimit is ecxeeded", func() {
							...
							When("the 'sent' address is not whitelisted", func() {
								It("Should revert", func() {
									...
									Expect(isSuccessful(tx)).To(BeFalse())
									Expect(TestRig.LastExecuted()).To(MatchRegexp(`.*require\(etherValue <= spendAvailable\(\), "transfer amount exceeds available spend limit"\);`))
								})
							})
