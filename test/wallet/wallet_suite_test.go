...
func SignData(nonce *big.Int, address common.Address, data []byte, prv *ecdsa.PrivateKey) ([]byte, error) {
	relayMessage := fmt.Sprintf("monolith:%s%s%s", abi.U256(nonce), address, data)
