...
var _ = Describe("addTokens", func() {
	...
	Context("When called by the controller", func() {
		Context("When the added tokens are not already supported", func() {
			...
			Context("When at least one of the added tokens is already supported", func() {
				BeforeEach(func() {
					...
					tx, err = oracle.AddTokens(controllerWallet.TransactOpts(WithGasLimit(200000)), tokens, stringsToByte32("BNT", "OMG"), []uint8{18, 18})
					Expect(err).ToNot(HaveOccurred())
					be.Commit()
				})
			...
			Context("When none of the added tokens are supported", func() {
				BeforeEach(func() {
					...
					tx, err = oracle.AddTokens(controllerWallet.TransactOpts(WithGasLimit(300000)), tokens, stringsToByte32("MKR", "MTL"), []uint8{1, 12})
					Expect(err).ToNot(HaveOccurred())
					be.Commit()
				})
		...
		Context("When the parameters have different lengths", func() {
			It("Should fail", func() {
				tokens := []common.Address{common.HexToAddress("0x0"), common.HexToAddress("0x1")}
				tx, err := oracle.AddTokens(controllerWallet.TransactOpts(WithGasLimit(300000)), tokens, stringsToByte32("BNT", "TKN", "ZRX"), []uint8{18, 8})
				Expect(err).ToNot(HaveOccurred())
				be.Commit()
	...
	Context("When called by a random address", func() {
		It("Should fail", func() {
			tokens := []common.Address{common.HexToAddress("0x0"), common.HexToAddress("0x1")}
			tx, err := oracle.AddTokens(randomWallet.TransactOpts(WithGasLimit(300000)), tokens, stringsToByte32("BNT", "TKN"), []uint8{18, 8})
			Expect(err).ToNot(HaveOccurred())
			be.Commit()
			Expect(isGasExhausted(tx, 300000)).To(BeFalse())
			Expect(isSuccessful(tx)).To(BeFalse())
