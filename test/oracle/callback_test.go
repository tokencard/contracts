			...
			Context("When a valid API pubKey is used", func() {
				Context("When a valid queryID is passed", func() {
					...
					Context("When the proof is not valid", func() {
						...
						Context("When the date update timestamp is not zero", func() {
							var timestamp *big.Int
							// BeforeEach(func() {
							// 	//update the rate manually in order to set the lastupdated field
							// 	be.AdjustTime(time.Now().Sub(time.Time{}))
							// 	be.Commit()
							// })
							BeforeEach(func() {
								//update the rate manually in order to set the lastupdated field
								tx, err := oracle.UpdateTokenRate(controller.TransactOpts(), common.HexToAddress("0xfe209bdE5CA32fa20E6728A005F26D651FFF5982"), big.NewInt(555))
								Expect(err).ToNot(HaveOccurred())
								be.Commit()
								Expect(isSuccessful(tx)).To(BeTrue())
							})
							It("Should update the token's rate and timestamp ", func() {
								...
								Expect(err).ToNot(HaveOccurred())
								Expect(token.Rate.String()).To(Equal(big.NewInt(555).String()))
								Expect(token.LastUpdate).NotTo(Equal(big.NewInt(0)))
							})
							Context("When the date is invalid", func() {
								...
								It("Should update the token's rate and timestamp ", func() {
									token, err := oracle.Tokens(nil, common.HexToAddress("0xfe209bdE5CA32fa20E6728A005F26D651FFF5982"))
									Expect(err).ToNot(HaveOccurred())
									Expect(token.Rate.String()).To(Equal(big.NewInt(1702000000000000).String()))
									// Expect(token.LastUpdate).NotTo(Equal(big.NewInt(0)))
						...
						}) //date update timestamp is not zero
