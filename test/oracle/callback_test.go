...
var _ = Describe("callback", func() {
	...
	Context("When called by a random account", func() {
		...
		Context("When a token exists", func() {
			BeforeEach(func() {
				...
				Context("When the proof is valid", func() {
					...
					Context("When the result has the expected format", func() {
						BeforeEach(func() {
							...
							tx, err := Oracle.Callback(RandomAccount.TransactOpts(), common.HexToAddress("0xfe209bdE5CA32fa20E6728A005F26D651FFF5982"), "{\"ETH\":0.001702}", proof)
							Expect(err).ToNot(HaveOccurred())
							Backend.Commit()
							Expect(isSuccessful(tx)).To(BeTrue())
						})
						...
						It("Should update the token's rate and timestamp ", func() {
							symbol, magnitude, rate, available, loadable, redeemable, lastUpdate, err := TokenWhitelist.GetTokenInfo(nil, common.HexToAddress("0xfe209bdE5CA32fa20E6728A005F26D651FFF5982"))
							Expect(err).ToNot(HaveOccurred())
						...
						It("Should fail when called again with the same (deleted, not valid) queryID", func() {
							proof := common.Hex2Bytes("")
							tx, err := Oracle.Callback(RandomAccount.TransactOpts(ethertest.WithGasLimit(100000)), common.HexToAddress("0xfe209bdE5CA32fa20E6728A005F26D651FFF5982"), "{\"ETH\":0.001702}", proof)
							Expect(err).ToNot(HaveOccurred())
							Backend.Commit()
					...
					When("the token is removed before the callback!!!", func() {
						...
						BeforeEach(func() {
							proof := common.Hex2Bytes("")
							tx, err := Oracle.Callback(RandomAccount.TransactOpts(ethertest.WithGasLimit(200000)), common.HexToAddress("0xfe209bdE5CA32fa20E6728A005F26D651FFF5982"), "{\"ETH\":0.001702}", proof)
							Expect(err).ToNot(HaveOccurred())
							Backend.Commit()
							Expect(isSuccessful(tx)).To(BeFalse())
					...
					Context("When the result is is misformated", func() {
						...
						It("It should Fail", func() {
							...
							tx, err := Oracle.Callback(RandomAccount.TransactOpts(ethertest.WithGasLimit(500000)), common.HexToAddress("0xfe209bdE5CA32fa20E6728A005F26D651FFF5982"), "{\"ETH\"=0.003637}", proof)
							Expect(err).ToNot(HaveOccurred())
							Backend.Commit()
							Expect(isSuccessful(tx)).To(BeFalse())
						...
						It("It should Fail", func() {
							...
							tx, err := Oracle.Callback(RandomAccount.TransactOpts(ethertest.WithGasLimit(500000)), common.HexToAddress("0xfe209bdE5CA32fa20E6728A005F26D651FFF5982"), "{\"ETH\"=0.003637 mpla mpla mpla mpla", proof)
							Expect(err).ToNot(HaveOccurred())
							Backend.Commit()
							Expect(isSuccessful(tx)).To(BeFalse())
						...
						It("It should Fail", func() {
							...
							tx, err := Oracle.Callback(RandomAccount.TransactOpts(ethertest.WithGasLimit(500000)), common.HexToAddress("0xfe209bdE5CA32fa20E6728A005F26D651FFF5982"), "chancellor on brink of second bailout for banks", proof)
							Expect(err).ToNot(HaveOccurred())
							Backend.Commit()
							Expect(isSuccessful(tx)).To(BeFalse())
							Expect(TestRig.LastExecuted()).To(MatchRegexp(`.*require\(jsonLen > 8 && jsonLen <= 28, "misformatted input"\);`))
						...
						It("It should Fail", func() {
							...
							tx, err := Oracle.Callback(RandomAccount.TransactOpts(ethertest.WithGasLimit(500000)), common.HexToAddress("0xfe209bdE5CA32fa20E6728A005F26D651FFF5982"), "{\"ETH\"=this result is too long and it is going to inject malicious code}", proof)
							Expect(err).ToNot(HaveOccurred())
							Backend.Commit()
							Expect(isSuccessful(tx)).To(BeFalse())
							Expect(TestRig.LastExecuted()).To(MatchRegexp(`.*require\(jsonLen > 8 && jsonLen <= 28, "misformatted input"\);`))
					...
					Context("When the date is is misformated", func() {
						...
						It("It should Fail", func() {
							...
							tx, err := Oracle.Callback(RandomAccount.TransactOpts(ethertest.WithGasLimit(300000)), common.HexToAddress("0xfe209bdE5CA32fa20E6728A005F26D651FFF5982"), "{\"ETH\":0.003637}", proof)
							Expect(err).ToNot(HaveOccurred())
							Backend.Commit()
							Expect(isSuccessful(tx)).To(BeFalse())
						...
						It("It should Fail", func() {
							...
							Backend.Commit()
							Expect(isSuccessful(tx)).To(BeFalse())
							Expect(TestRig.LastExecuted()).To(MatchRegexp(`.*revert\("not a valid month"\);`))
						})
						...
						It("It should Fail", func() {
							...
							Backend.Commit()
							Expect(isSuccessful(tx)).To(BeFalse())
							Expect(TestRig.LastExecuted()).To(MatchRegexp(`.*revert\("invalid digit"\);`))
						})
						...
						It("It should Fail", func() {
							...
							Backend.Commit()
							Expect(isSuccessful(tx)).To(BeFalse())
							Expect(TestRig.LastExecuted()).To(MatchRegexp(`.*require\(expIndex \+ 1 == i, "- sign not immediately after e"\);`))
						})
				...
				Context("When the proof is not valid", func() {
					...
					Context("When the date is invalid", func() {
						...
						BeforeEach(func() {
							...
							tx, err := Oracle.Callback(RandomAccount.TransactOpts(ethertest.WithGasLimit(300000)), common.HexToAddress("0xfe209bdE5CA32fa20E6728A005F26D651FFF5982"), "{\"ETH\":0.001702}", proof)
							Expect(err).ToNot(HaveOccurred())
							Backend.Commit()
							Expect(isSuccessful(tx)).To(BeFalse())
						...
						Context("When the signature is valid but the proof is not", func() {
							...
							Context("When the result hash is not matching", func() {
								...
								It("It should Fail", func() {
									...
									tx, err := Oracle.Callback(RandomAccount.TransactOpts(ethertest.WithGasLimit(300000)), common.HexToAddress("0xfe209bdE5CA32fa20E6728A005F26D651FFF5982"), "{\"ETH\":Moon}", proof)
									Expect(err).ToNot(HaveOccurred())
									Backend.Commit()
									Expect(isSuccessful(tx)).To(BeFalse())
