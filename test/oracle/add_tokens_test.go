...
var _ = Describe("addTokens", func() {
	...
	Context("When called by the controller", func() {
		Context("When the added tokens are not already supported", func() {
			...
			BeforeEach(func() {
				...
				tx, err = oracle.AddTokens(controller.TransactOpts(), tokens, stringsToByte32("BNT", "TKN"), []*big.Int{getMagnitude(big.NewInt(18)), getMagnitude(big.NewInt(8))})
			...
			It("Should update the tokens map", func() {
				...
				Expect(token.Magnitude).To(Equal(getMagnitude(big.NewInt(18))))
			...
			Context("When at least one of the added tokens is already supported", func() {
				It("Should fail", func() {
					...
					tx, err = oracle.AddTokens(controller.TransactOpts(WithGasLimit(200000)), tokens, stringsToByte32("BNT", "OMG"), []*big.Int{getMagnitude(big.NewInt(18)), getMagnitude(big.NewInt(18))})
					Expect(err).ToNot(HaveOccurred())
					be.Commit()
			...
			Context("When none of the added tokens are supported", func() {
				It("Should pass", func() {
					...
					tx, err = oracle.AddTokens(controller.TransactOpts(WithGasLimit(300000)), tokens, stringsToByte32("MKR", "MTL"), []*big.Int{getMagnitude(big.NewInt(18)), getMagnitude(big.NewInt(8))})
					Expect(err).ToNot(HaveOccurred())
					be.Commit()
		...
		Context("When the parameters have different lengths", func() {
			It("Should fail", func() {
				tokens := []common.Address{common.HexToAddress("0x4"), common.HexToAddress("0x5")}
				tx, err := oracle.AddTokens(controller.TransactOpts(WithGasLimit(300000)), tokens, stringsToByte32("BNT", "TKN", "ZRX"), []*big.Int{getMagnitude(big.NewInt(18)), getMagnitude(big.NewInt(8))})
				Expect(err).ToNot(HaveOccurred())
				be.Commit()
				Expect(isGasExhausted(tx, 300000)).To(BeFalse())
				Expect(isSuccessful(tx)).To(BeFalse())
	...
	Context("When called by a random address", func() {
		It("Should fail", func() {
			tokens := []common.Address{common.HexToAddress("0x0"), common.HexToAddress("0x1")}
			tx, err := oracle.AddTokens(randomAccount.TransactOpts(WithGasLimit(300000)), tokens, stringsToByte32("BNT", "TKN"), []*big.Int{getMagnitude(big.NewInt(18)), getMagnitude(big.NewInt(8))})
			Expect(err).ToNot(HaveOccurred())
			be.Commit()
			Expect(isGasExhausted(tx, 300000)).To(BeFalse())
			Expect(isSuccessful(tx)).To(BeFalse())
