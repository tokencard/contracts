...
var _ = Describe("withdrawTokens", func() {
  ...
  When("The Licence contract is credited with 333 of ERC20 type-1 tokens and 1 ETH", func() {
    ...
    When("the owner withdraws 222 tokens and 1 ETH", func() {
      ...
      BeforeEach(func() {
  			tx, err := Licence.WithdrawTokens(Owner.TransactOpts(), RandomAccount.Address(), ERC20Contract1Address, big.NewInt(222))
        Expect(err).ToNot(HaveOccurred())
  			Backend.Commit()
  			Expect(isSuccessful(tx)).To(BeTrue())
  		})
      ...
      BeforeEach(func() {
  			tx, err := Licence.WithdrawTokens(Owner.TransactOpts(), RandomAccount.Address(), common.HexToAddress("0x0"), EthToWei(1))
        Expect(err).ToNot(HaveOccurred())
  			Backend.Commit()
  			Expect(isSuccessful(tx)).To(BeTrue())
  		})
      ...
      It("Should emit 2 Transferred events", func() {
        it, err := Licence.FilterTransferred(nil)
        Expect(err).ToNot(HaveOccurred())
        Expect(it.Next()).To(BeTrue())
        evt := it.Event
        Expect(it.Next()).To(BeTrue())
        Expect(evt.To).To(Equal(RandomAccount.Address()))
        Expect(evt.Token).To(Equal(ERC20Contract1Address))
        Expect(evt.Amount.String()).To(Equal("222"))
        evt = it.Event
        Expect(it.Next()).To(BeFalse())
        Expect(evt.To).To(Equal(RandomAccount.Address()))
        Expect(evt.Token).To(Equal(common.HexToAddress("0x0"))) //represents ETH
